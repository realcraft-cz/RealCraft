...
public class AntiSpam implements Listener {
    ...
    public void loadConfig() {
        if (plugin.config.getBoolean("antispam.enabled")) {
            ...
            if (spamMessage.length() == 0 || spamMessage.equals("false"))
                spamMessage = null;
    ...
    public void onPlayerChat(AsyncPlayerChatEvent event) {
        ...
        if (lastMessageTime + timeLimit >= System.currentTimeMillis()) {
            if (spamMessage != null)
                player.sendMessage(RealCraft.parseColors(spamMessage));
        ...
        /** Repeated chars */
        message = message.replaceAll("(.)\\1{" + maxSameChars + ",}", "$1$1$1$1$1");
        message = message.replaceAll("(..)\\1{" + maxSameChars + ",}", "$1$1$1$1$1");
