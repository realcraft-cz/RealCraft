...
public class ShopMarket implements Runnable {
	...
	public void loadMarkets(){
		markets.clear();
		ResultSet rs = DB.query("SELECT t1.*,t2.user_name FROM "+MARKETS+" t1 INNER JOIN authme t2 USING(user_id)");
		try {
			while(rs.next()){
	...
	public static void createMarket(Player player,int price){
		...
		if(residence == null || !residence.isOwner(player)){
			...
			ShopManager.sendMessage(player,"§cMinimalni nabidka je "+Economy.format(ShopMarket.getMinimumPrice()));
			return;
		}
		...
		else if(Users.getUser(player).getMoney() < price){
			...
			DB.update("INSERT INTO "+MARKETS+" (user_id,market_price,market_residence,market_lastpaid) VALUES('"+market.getOwner()+"','"+market.getPrice()+"','"+market.getResidence()+"','"+market.getLastPaid()+"')");
			ShopMarket.updateMarkets();
		}
	}
	...
	public static class ShopMarketResidence {
		...
		public void checkLastPaid(){
			...
			if(lastPaid > yesterday && lastPaid < yesterday+86400){
				...
				if(player != null && Users.getUser(player).getId() == this.getOwner()){
					...
					DB.update("UPDATE "+MARKETS+" SET market_lastpaid = '"+lastPaid+"' WHERE user_id = '"+this.getOwner()+"'");
		...
		public void rename(String residence){
			this.residence = residence;
			DB.update("UPDATE "+MARKETS+" SET market_residence = '"+residence+"' WHERE user_id = '"+this.getOwner()+"'");
		}

		public void remove(){
			DB.update("DELETE FROM "+MARKETS+" WHERE user_id = '"+this.getOwner()+"'");
		}
	}
	...
	public static class ShopMarketPlace {
		...
		public void showInfo(Player player){
			if(market != null){
				...
				player.sendMessage("§6Cena za propagaci: §a"+Economy.format(market.getPrice()));
			}
		}
	}
}