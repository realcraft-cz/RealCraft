...
public class MapCommandInfo extends MapCommand {
	...
	public void perform(Player player,String[] args){
		...
		player.sendMessage("§7§m"+StringUtils.repeat(" ",10)+"§r §f§l"+map.getName()+" §7[#"+map.getId()+"] ("+map.getType().getColor()+map.getType().getName()+"§7) §7§m"+StringUtils.repeat(" ",47-+(map.getName()+" §7[#"+map.getId()+"] ("+map.getType().getColor()+map.getType().getName()+"§7)").length()));
		player.sendMessage("§7Autor: "+map.getUser().getRank().getChatColor()+"§l"+map.getUser().getName());
		if(trusted.length > 0) player.sendMessage("§7Trusted: "+StringUtils.join(trusted,", "));
