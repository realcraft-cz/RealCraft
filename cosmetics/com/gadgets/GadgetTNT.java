...
public class GadgetTNT extends Gadget {
	...
	public GadgetTNT(GadgetType type){
		super(type);
	}
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        if (entities.contains(event.getEntity())){
            ...
            for(Entity ent : event.getEntity().getNearbyEntities(3, 3, 3)){
                if((RealCraft.getInstance().lobby.lobbycitizens == null || !RealCraft.getInstance().lobby.lobbycitizens.npcRegistry.isNPC(ent)) && (ent instanceof Creature || ent instanceof Player)){
                    ...
                    AntiCheat.exempt((Player)ent,2000);
                }
            }
